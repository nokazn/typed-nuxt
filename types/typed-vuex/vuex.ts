...
declare module 'typed-vuex' {
  ...
  type ExtendedCommit<M> = <T extends keyof (M & RootMutations)>(
    type: T,
  ...
  // ローカルモジュール内ではモジュール名なしで指定できる
  type ExtendedDispatch<A extends ActionMethodMap> = <
    T extends keyof (A & RootActions)
  >(
    type: T,
  ...
  type ExtendedSubscribe = <
    P extends ExtendedMutationPayload<keyof RootMutations>
  >(
